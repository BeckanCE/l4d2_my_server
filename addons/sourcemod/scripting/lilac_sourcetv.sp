#include <sourcemod>
#include <multicolors> // <--- AÑADIDO: Necesario para usar colores en el chat
...
public Plugin:myinfo = {
	...
	version = "1.2.2", // Version incrementada
	url = ""
};
...
void update_recording_list(int client, bool status)
{
	...
	if (players == 0 && stv_recording) {
		...
		if (icvar[CVAR_RECORD]) {
			...
			// <--- AÑADIDO: Imprimir la información de los jugadores a los espectadores
			CreateTimer(1.0, timer_print_player_info); // Usamos un timer para asegurar que el mensaje aparezca después de la confirmación de grabación
		}
...
// <--- AÑADIDO: Timer para llamar a la función de impresión
public Action timer_print_player_info(Handle timer)
{
    print_player_info_to_spectators();
    return Plugin_Continue;
}


// <--- AÑADIDO: Nueva función para imprimir datos en el chat de los espectadores
void print_player_info_to_spectators()
{
	// Recorremos todos los posibles clientes para encontrar a los espectadores
	for (int i = 1; i <= MaxClients; i++)
	{
		// Nos aseguramos de que el cliente esté en el juego y sea un espectador o SourceTV
		if (IsClientInGame(i) && (GetClientTeam(i) == 1 || IsClientSourceTV(i)))
		{
			CPrintToChat(i, "{default}[{green}!{default}] {blue}Datos de jugadores:");
			
			// Ahora recorremos de nuevo la lista para encontrar a los jugadores activos
			for (int j = 1; j <= MaxClients; j++)
			{
				// Verificamos que sea un jugador válido (no bot, en un equipo)
				if (is_player_valid(j) && !IsFakeClient(j) && (GetClientTeam(j) == 2 || GetClientTeam(j) == 3))
				{
					char name[MAX_NAME_LENGTH];
					char steamid[64];
					
					GetClientName(j, name, sizeof(name));
					GetClientAuthId(j, AuthId_Steam2, steamid, sizeof(steamid));
					
					// Imprimimos la línea formateada con colores al cliente espectador (i)
					CPrintToChat(i, "{red}-> {green}%s {default}- {olive}%s", name, steamid);
				}
			}
		}
	}
